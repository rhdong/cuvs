...
class cuvs_cagra_merge : public algo<T>, public algo_gpu {
 public:
  ...
  void build(const T* dataset, size_t nrow) override
  {
    ...
    for (int i = 0; i < build_param_.n_splits && offset < nrow; ++i) {
      ...
      auto idx     = std::make_unique<cuvs::neighbors::cagra::index<T, IdxT>>(std::move(
    ...
    if (build_param_.strategy ==
        cuvs::neighbors::cagra::MergeStrategy::MERGE_STRATEGY_PHYSICAL) {
      index_ = std::make_unique<cuvs::neighbors::cagra::index<T, IdxT>>(
        cuvs::neighbors::cagra::merge(handle_, merge_param_, ptrs));
    } else {
      composite_index_ = std::make_unique<cuvs::neighbors::cagra::composite_index<T, IdxT>>(
  ...
  void set_search_param(const search_param_base& param, const void* filter_bitset) override
  {
    auto sp  = dynamic_cast<const search_param&>(param);
    sp_      = sp;
    filter_  = make_cuvs_filter(filter_bitset, index_ ? index_->size() : 0);
  }
  ...
  {
    index_ = std::make_unique<cuvs::neighbors::cagra::index<T, IdxT>>(handle_);
    cuvs::neighbors::cagra::deserialize(handle_, file, index_.get());
  }
 ...
 private:
  ...
  search_param sp_{};
  std::vector<std::unique_ptr<cuvs::neighbors::cagra::index<T, IdxT>>> sub_indices_;
  std::unique_ptr<cuvs::neighbors::cagra::index<T, IdxT>> index_;
  std::unique_ptr<cuvs::neighbors::cagra::composite_index<T, IdxT>> composite_index_;
  std::shared_ptr<cuvs::neighbors::filtering::base_filter> filter_;
};

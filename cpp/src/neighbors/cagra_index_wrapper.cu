...
IndexWrapper<T, IdxT, OutputIdxT>::merge(
  ...
  const std::vector<
    ...
    other_indices) const
{
  // Try to cast to CAGRA merge params
  ...
  if (cagra_params->strategy() == cuvs::neighbors::MergeStrategy::MERGE_STRATEGY_LOGICAL) {
    // Logical merge: create CompositeIndex with wrapped indices
    ...
    // Physical merge: call native CAGRA merge

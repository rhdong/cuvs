...
**Macros.** We define the macros `RAFT_COMPILED` and `CUVS_EXPLICIT_INSTANTIATE_ONLY`. The `RAFT_COMPILED` macro is defined by `CMake` when compiling code that (1) is part of `libraft.so` or (2) is linked with `libraft.so`. It indicates that a precompiled `libraft.so` is present at runtime.
...
Below, we summarize which combinations of `RAFT_COMPILED` and `CUVS_EXPLICIT_INSTANTIATE_ONLY` are used in practice and what the effect of the combination is. 

| RAFT_COMPILED | CUVS_EXPLICIT_INSTANTIATE_ONLY | Which targets                                                                                        |
|---------------|--------------------------------|------------------------------------------------------------------------------------------------------|
...
| RAFT_COMPILED | CUVS_EXPLICIT_INSTANTIATE_ONLY | Effect                                                                                                |
|---------------|--------------------------------|-------------------------------------------------------------------------------------------------------|
...
3. The order of inclusion in `expensive.cuh` is extremely important. If `CUVS_EXPLICIT_INSTANTIATE_ONLY` is not defined, but `RAFT_COMPILED` is defined, then we must include the template definitions before the `extern template` instantiations.
